...
const DeleteAccountRequests = () => {
  ...
  const fetchDeleteRequests = async () => {
    try {
      ...
      if (data.success) {
        ...
        const all = data.data || [];
        const users = all.filter(req => req.userId);
        const vendors = all.filter(req => req.adminId && req.adminId.shopId);
  ...
  const formatDate = (dob) => {
    ...
    // If DOB is like "23-12-2003" (DD-MM-YYYY)
    if (dob.includes("-")) {
  ...
  const userRequests = requests.users || [];
  return (
    <div className="page-container">


      <div className="page-header">
        <h1 className="page-title mb-0">User Delete Account Requests</h1>
        <button
